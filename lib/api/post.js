...
// Re-export for backward compatibility if anything imports NO_STORE from here
export { NO_STORE } from "@/lib/api-base";

export const DEBUG = false; // set to true only if you want console logs

// Resolved once at module load; adjust to call getApiBase() inline if you prefer per-request
...
function isLikelyId(token){ return /^[a-f0-9-]{16,}$/i.test(String(token)); } // uuid/guid-ish
...
export async function fetchPostByIdOrSlug({ idOrSlug, site }){
  ...
  const urls = buildCandidates(API_BASE, idOrSlug, siteKey);
      ...
      if (data && typeof data === "object" && !Array.isArray(data)) {
        ...
        if (DEBUG) {
          console.log("[post][unwrap]", {
            ...
            cand0Keys: cands[0] ? Object.keys(cands[0]).slice(0,12) : [],
          });
        }
      ...
      if (arr.length > 0) {
        if (DEBUG) {
          console.log("[post][array]", {
            length: arr.length,
            firstKeys: Object.keys(arr[0] || {}).slice(0,12),
          });
        }
  ...
  throw lastErr || new Error(`[post] Not found for: ${idOrSlug} (site=${siteKey})`);
}
