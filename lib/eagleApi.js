...
/* ------------------------------- home: local ------------------------------- */
/**
 * Fetch home feed for a site + categories (e.g., local)
...
export async function fetchHomeRegional({ site, limit = 12 } = {}) {
  ...
  try {
    ...
    // Prefer categories array named "state"
    const categories = json?.data?.categories || [];
    const regionalArr =
...
export async function fetchHomeFeatured({ site } = {}) {
  ...
  try {
    ...
    const featuredArr = cats.find((c) => (c?.title || "").toLowerCase() === "featured")?.data || [];
...
export async function fetchHomeMore({
  ...
  try {
    const multiUrl = `${BASE}/home?site=${encodeURIComponent(siteLower)}&categories=${encodeURIComponent(
      cats.join(",")
    )}`;
    if (debug) console.log("[fetchHomeMore] multi URL:", multiUrl);

    const res = await fetch(multiUrl, NO_STORE);
    if (res.ok) {
      const json = await res.json();
  ...
  if (!all || all.length === 0) {
    const perCat = await Promise.allSettled(
      cats.map(async (cat) => {
        const url = `${BASE}/home?site=${encodeURIComponent(siteLower)}&categories=${encodeURIComponent(cat)}`;
        if (debug) console.log("[fetchHomeMore] per-cat URL:", url);
    ...
    all = perCat.filter((r) => r.status === "fulfilled").flatMap((r) => r.value);
  }
...
// Normalize one post into our app shape (used by fetchArticleById)
