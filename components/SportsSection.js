export default function SportsSection({ items = [] }) {
  const keyFor = (it, idx) =>
    it.id || it.slug || it.href || it.title || `idx-${idx}`;

  return (
  <section aria-labelledby="sports-heading" className="pb-20">
    <h2 id="sports-heading" className="section-underline-sports font-bold text-[32px]">
      Sports
    </h2>
