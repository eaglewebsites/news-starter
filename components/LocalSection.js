export default function LocalSection({ items = [] }) {
  const keyFor = (it, idx) =>
    it.id || it.slug || it.href || it.title || `idx-${idx}`;

  return (
  <section aria-labelledby="local-heading" className="pb-20">
    <h2 id="local-heading" className="section-underline-local font-bold text-[32px]">
      Local
    </h2>
